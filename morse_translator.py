...
class Translator(ttk.Frame):
    def __init__(self, parent, **kwargs):
        ...
        self.destino_lbl = ttk.Label(self, text="Morse", width = 6, anchor=W)
    ...
    def traduce(self):
        texto_original = self.origin_text.get(1) #Aquí hay un error hay que conseguir el texto de self.origin_text
    ...
    def changeText(self):
        if self.traduccionDirecta:
            ...
            self.destino_lbl.config = "Plano"
        else:
            self.origin_lbl.config(text = "Plano")
            self.destino.lbl.config = "Morse"
